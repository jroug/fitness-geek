...
export default function EnterPage() {
  ...
  const login = async (username: string, password: string) => {
    try {
      ...
      setloginErrorBorder(true);
      setLoginErrorMessage("Unable to sign in right now. Please try again.");
    } finally {
        ...
        <section className="bg-[#f4f5f7] px-6 py-10 sm:px-10 lg:px-12">
          <div className="mx-auto flex h-full w-full max-w-[420px] flex-col justify-center">
            <div className="mb-8">
              ...
              <p className="mt-2 text-sm text-slate-500">Your Fitness Dashboard</p>
            </div>
            ...
            <form className="space-y-4" onSubmit={handleSignInSubmit}>
              <div>
                ...
                <input
                  type="email"
                  id="email"
                  className={`w-full rounded-lg border bg-white px-3 py-2.5 text-sm text-slate-900 outline-none transition focus:border-cyan-600 ${
                    loginErrorBorder ? "border-rose-400" : "border-slate-300"
                  }`}
              ...
              <button
                ...
                className="w-full rounded-lg bg-gradient-to-r from-sky-700 to-cyan-600 px-4 py-2.5 text-sm font-semibold text-white transition hover:from-sky-800 hover:to-cyan-700 disabled:cursor-not-allowed disabled:opacity-70"
              >
            ...
            <div className="mt-4 min-h-[20px]">
              <p className="text-sm text-rose-500">{loginErrorMessage}</p>
            </div>

            <p className="mt-2 text-center text-sm text-slate-500">
              New to Fitness Geek?{" "}
              <Link href="/users/join" className="font-semibold text-cyan-700 hover:text-cyan-600">
                Create an account
              </Link>
            </p>
          </div>
        ...
        <section className="relative hidden overflow-hidden bg-[#032846] text-white xl:flex lg:items-center">
          ...
          <div className="relative z-10 mx-auto w-full max-w-[560px] px-12">
            ...
            <p className="mt-6 max-w-[420px] text-xl text-slate-200">
              Fitness Geek helps you track workouts, nutrition, and progress in one place so you can stay consistent
              every week.
            </p>
            <Link
              href="/features"
